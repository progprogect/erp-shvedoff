-- Миграция: Расширение модели товара с поддержкой краёв и бортов (edges_v2) - ФИНАЛЬНАЯ
-- Дата: 2025-08-18
...
-- Создаем ENUM для типов краёв (без IF NOT EXISTS для совместимости)
DO $$ BEGIN
    CREATE TYPE edge_type AS ENUM ('puzzle', 'straight', 'plain');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Создаем ENUM для сторон краёв (без IF NOT EXISTS для совместимости)
DO $$ BEGIN
    CREATE TYPE edge_side AS ENUM ('top', 'bottom', 'left', 'right');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- Индексы для JSONB полей dimensions
CREATE INDEX IF NOT EXISTS idx_products_dimensions_length ON products USING GIN ((dimensions->>'length'));
CREATE INDEX IF NOT EXISTS idx_products_dimensions_width ON products USING GIN ((dimensions->>'width'));
CREATE INDEX IF NOT EXISTS idx_products_dimensions_thickness ON products USING GIN ((dimensions->>'thickness'));
...
SELECT '=== ПРОВЕРКА НОВЫХ ПОЛЕЙ ===' as status;
SELECT column_name, data_type, is_nullable, column_default 
FROM information_schema.columns 
WHERE table_name = 'products' AND column_name IN ('height_mm', 'bottom_type_id')
